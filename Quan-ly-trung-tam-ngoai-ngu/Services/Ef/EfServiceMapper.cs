...
internal static class EfServiceMapper
{
    ...
    internal static string MapCourseStatus(byte status, DateTime? nextStartDate)
    {
        if (status != 1)
        {
            return "Tạm dừng";
        ...
        return diffDays > 7 ? "Đang tuyển sinh" : "Đang hoạt động";
    }

    internal static string MapClassStatus(byte status, DateTime startDate, DateTime endDate, int enrolled, int capacity)
    {
        if (status != 1)
        {
            return "Tạm dừng";
    ...
    internal static string MapEnrollmentStatus(string status, DateTime? classStartDate)
    {
    ...
    internal static string MapPaymentStatus(decimal totalFee, decimal paidAmount)
    {
    ...
    internal static string MapPaymentMethod(string paymentMethod)
    {
    ...
    internal static DateTime CalculateDebtDueDate(DateTime enrollDate, DateTime? classStartDate)
    {
    ...
    internal static string MapDebtStatus(DateTime dueDate)
    {
    ...
    internal static string MapSessionStatus(DateTime sessionDate)
    {
    ...
    internal static string MapAttendanceStatus(string status)
    {
        return status switch
        {
            "Present" => "Có mặt",
    ...
    internal static string BuildExamLabel(string examType, string examName)
    {
    ...
    internal static string MapExamResult(string? resultStatus, decimal score, decimal averageScore)
    {
    ...
    internal static string InferCourseLevel(string courseName)
    {
    ...
    internal static string BuildShortDescription(string? description, string courseName)
    {
        ...
        return description.Length <= maxLength ? description : $"{description[..maxLength].Trim()}...";
    }

    internal static string BuildCourseTarget(string courseName, int durationHours)
    {
        if (durationHours <= 0)
        {
    ...
    internal static List<string> BuildCourseObjectives(string courseName)
    {
        var normalized = RemoveDiacritics(courseName).ToLowerInvariant();
    ...
    internal static List<string> BuildCourseHighlights(string courseName)
    {
        var normalized = RemoveDiacritics(courseName).ToLowerInvariant();
        if (normalized.Contains("ielts"))
        {
