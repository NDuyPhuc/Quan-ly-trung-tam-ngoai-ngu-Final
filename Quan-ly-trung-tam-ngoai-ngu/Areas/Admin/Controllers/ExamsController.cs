...
public class ExamsController : AdminControllerBase
{
    ...
    public IActionResult Index()
    {
        ...
        {
            Title = "Bài kiểm tra",
            Subtitle = "Quản lý danh sách bài kiểm tra độc lập với màn hình nhập kết quả học tập.",
            Breadcrumbs = Breadcrumbs("Bài kiểm tra"),
            PrimaryActionText = "Tạo bài kiểm tra",
            PrimaryActionUrl = "/Admin/Exams/Create",
            SearchPlaceholder = "Tìm theo lớp, tên bài kiểm tra hoặc loại bài kiểm tra...",
            SummaryCards =
            [
                new SummaryCardViewModel { Title = "Tổng bài kiểm tra", Value = exams.Count.ToString(), Description = "Đã tạo trong hệ thống", Icon = "bi-journal-text", AccentClass = "primary" },
                new SummaryCardViewModel { Title = "Đã có kết quả", Value = exams.Count(x => x.ResultCount > 0).ToString(), Description = "Đã phát sinh điểm số", Icon = "bi-clipboard2-check", AccentClass = "success" },
                new SummaryCardViewModel { Title = "Chưa nhập điểm", Value = exams.Count(x => x.ResultCount == 0).ToString(), Description = "Sẵn sàng cho giáo viên nhập điểm", Icon = "bi-hourglass-split", AccentClass = "warning" }
            ],
            Table = new TableViewModel
            {
                ...
                Rows = exams.Select(item => new TableRowViewModel
                {
                    ...
                    Actions =
                    [
                        new() { Label = "Chi tiết", Url = $"/Admin/Exams/Details/{item.Id}", Icon = "bi-eye" },
                        new() { Label = "Sửa", Url = $"/Admin/Exams/Edit/{item.Id}", Icon = "bi-pencil-square", CssClass = "btn btn-sm btn-outline-secondary" },
                        new() { Label = "Nhập điểm", Url = "/Admin/ExamResults/Create", Icon = "bi-clipboard2-data", CssClass = "btn btn-sm btn-outline-primary" },
                        new() { Label = "Xóa", Url = $"/Admin/Exams/Delete/{item.Id}", Icon = "bi-trash", CssClass = "btn btn-sm btn-outline-danger confirm-action", RequiresConfirm = true, ConfirmMessage = "Bạn muốn xóa bài kiểm tra này?" }
                    ]
                }).ToList()
            }
        });
    ...
    {
        return ManagementFormView(BuildExamForm("Tạo bài kiểm tra", "/Admin/Exams/Create", new ExamInput { ExamDate = DateTime.Today, MaxScore = 10 }));
    }
    ...
    public IActionResult Create(ExamInput input)
    {
        ...
        if (!result.Succeeded)
        {
            return ManagementFormView(BuildExamForm("Tạo bài kiểm tra", "/Admin/Exams/Create", input, result.Message));
        }
    ...
    public IActionResult Edit(int id)
    {
        ...
        return ManagementFormView(BuildExamForm("Cập nhật bài kiểm tra", $"/Admin/Exams/Edit/{id}", input));
    }
    ...
    public IActionResult Edit(int id, ExamInput input)
    {
        ...
        if (!result.Succeeded)
        {
            return ManagementFormView(BuildExamForm("Cập nhật bài kiểm tra", $"/Admin/Exams/Edit/{id}", input, result.Message));
        }
    ...
    {
        var item = DataService.GetExams().FirstOrDefault(x => x.Id == id);
        if (item is null)
        {
            SetToast("Không tìm thấy bài kiểm tra.", "danger");
            return RedirectToAction(nameof(Index));
        ...
        return ManagementDetailsView(new ManagementDetailsPageViewModel
        {
            ...
            Breadcrumbs = Breadcrumbs("Chi tiết bài kiểm tra", "Bài kiểm tra", "/Admin/Exams"),
            SummaryCards =
            [
                ...
                new SummaryCardViewModel { Title = "Số kết quả", Value = item.ResultCount.ToString(), Description = "Dòng điểm đã ghi nhận", Icon = "bi-clipboard2-check", AccentClass = "info" },
            ...
            Actions =
            [
                new QuickActionViewModel { Label = "Nhập điểm", Url = "/Admin/ExamResults/Create", Icon = "bi-clipboard2-data" },
                new QuickActionViewModel { Label = "Sửa bài kiểm tra", Url = $"/Admin/Exams/Edit/{id}", Icon = "bi-pencil-square", CssClass = "btn btn-outline-primary" },
                new QuickActionViewModel { Label = "Quay lại", Url = "/Admin/Exams", Icon = "bi-arrow-left", CssClass = "btn btn-outline-secondary" }
            ]
        });
    }
    ...
    {
        var classOptions = DataService.GetClasses()
            .OrderBy(x => x.Code)
        ...
        {
            Title = title,
            Subtitle = "Tạo riêng danh mục bài kiểm tra để giáo viên và quản trị nhập kết quả theo đúng cấu trúc dữ liệu.",
            Breadcrumbs = Breadcrumbs(title, "Bài kiểm tra", "/Admin/Exams"),
            FormTitle = title,
            FormDescription = "Thông tin sẽ được lưu vào bảng Exams. Kết quả điểm số sẽ được quản lý ở module ExamResults.",
            FormActionUrl = actionUrl,
            CancelUrl = "/Admin/Exams",
